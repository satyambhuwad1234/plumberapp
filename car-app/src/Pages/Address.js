...
const Address = ({ editAddress, setEditAddress, formData, setFormData, setAddresses, userId }) => {
    useEffect(() => {
        if (editAddress) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editAddress) {
                ...
                await axios.put(`http://localhost:5000/api/address/${editAddress._id}`, dataToSubmit);
                alert("Address updated successfully!");
            } else {
                // Add a new address
                const response = await axios.post("http://localhost:5000/api/address", dataToSubmit);
